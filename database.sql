...
IF OBJECT_ID('dbo.NOTA_SAIDA', 'U') IS NOT NULL
BEGIN
    ALTER TABLE NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_NOTASAIDA_CLIENTE;
    ALTER TABLE NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_NOTASAIDA_CONDPAG;
    ALTER TABLE NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_NOTASAIDA_USR_ALT;
END
GO
...
IF OBJECT_ID('dbo.CONTAS_A_RECEBER', 'U') IS NOT NULL
BEGIN
    ALTER TABLE CONTAS_A_RECEBER DROP CONSTRAINT IF EXISTS FK_ContasAReceber_NotaSaida;
    ALTER TABLE CONTAS_A_RECEBER DROP CONSTRAINT IF EXISTS FK_ContasAReceber_Cliente;
    ALTER TABLE CONTAS_A_RECEBER DROP CONSTRAINT IF EXISTS FK_ContasAReceber_FormaPagamento;
END
GO
IF OBJECT_ID('dbo.ITENS_NOTA_SAIDA', 'U') IS NOT NULL
BEGIN
    -- derruba FKs, se existirem
    ALTER TABLE ITENS_NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_ITEMNOTASAIDA_NOTASAIDA;
    ALTER TABLE ITENS_NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_ITEMNOTASAIDA_PRODUTO;
    ALTER TABLE ITENS_NOTA_SAIDA DROP CONSTRAINT IF EXISTS FK_ITEMNOTASAIDA_USR_ALT;
END
GO
...
DROP TABLE IF EXISTS ITENS_NOTA_SAIDA;
DROP TABLE IF EXISTS NOTA_SAIDA;
...
CREATE TABLE FUNCIONARIO (
    ...
    FUNCIONARIO_SALBRUTO DECIMAL(18,2),
    FUNCIONARIO_GRATPROD DECIMAL(18,2),
    FUNCIONARIO_SALLIQ DECIMAL(18,2),
...
CREATE TABLE CLIENTE (
    ...
    CLIENTE_LIMITE_CREDITO DECIMAL(18,2),
    CIDADE_ID INT NOT NULL,
    CONDPAG_ID INT NOT NULL,
    ATIVO BIT DEFAULT 1,
...
(
    -- Chave primária “surrogada”, padrão do projeto
    CONTA_RECEBER_ID       INT IDENTITY(1,1) PRIMARY KEY,

    -- Referências/entidades
    NOTA_SAIDA_ID          INT           NULL,          -- pode ser nulo para títulos avulsos
    ID_CLIENTE             INT           NOT NULL,      -- Contas a receber precisam de cliente
    NUMERO_PARCELA         INT           NOT NULL DEFAULT 1,

    -- Datas e valores (espelhando Contas_A_Pagar + campos da classe)
    DATA_EMISSAO           DATE          NOT NULL,
    DATA_VENCIMENTO        DATE          NOT NULL,
    VALOR_PARCELA          DECIMAL(18,2) NOT NULL,

    ID_FORMA_PAGAMENTO     INT           NOT NULL,
    SITUACAO               INT           NOT NULL DEFAULT 0,   -- 0: Em aberto, 1: Pago, 2+: cancelar/outros status do seu domínio
    ...
    -- Valores apurados (null quando não aplicável)
    VALOR_PAGO             DECIMAL(18,2) NULL,
    MULTA_VALOR            DECIMAL(18,2) NULL,
    JUROS_VALOR            DECIMAL(18,2) NULL,
    DESCONTO_VALOR         DECIMAL(18,2) NULL,

    DATA_PAGAMENTO         DATETIME      NULL,
    MOTIVO_CANCELAMENTO    VARCHAR(100)  NULL,

    -- Colunas padrão de auditoria (seguindo estilo da sua CONTAS_A_PAGAR)
    ATIVO                  BIT           NOT NULL DEFAULT 1,
    DATA_CADASTRO          DATETIME      NOT NULL DEFAULT GETDATE(),
    DATA_ULTIMA_EDICAO     DATETIME      NULL
);
...
CREATE TABLE NOTA_SAIDA (
    NOTA_SAIDA_ID               INT IDENTITY(1,1) PRIMARY KEY,

    -- “chave lógica” da nota
    NOTA_SAIDA_MODELO           VARCHAR(10) NOT NULL,
    NOTA_SAIDA_SERIE            VARCHAR(10) NOT NULL,
    NOTA_SAIDA_NUMERO           VARCHAR(50) NOT NULL,

    -- datas e status
    NOTA_SAIDA_DT_EMISSAO       DATETIME    NOT NULL,
    NOTA_SAIDA_MOT_CANCELAMENTO VARCHAR(99) NULL,

    -- relacionamentos
    CLIENTE_ID                  INT NOT NULL,
    FUNCIONARIO_ID              INT NOT NULL,   -- << NOVO
    CONDPAG_ID                  INT NOT NULL,

    -- auditoria
    ATIVO                       BIT          NOT NULL DEFAULT 1,
    NOTA_SAIDA_DT_CRIACAO       DATETIME     NOT NULL DEFAULT GETDATE(),
    NOTA_SAIDA_DT_ALT           DATETIME     NULL,
    NOTA_SAIDA_USR_ALT          INT          NULL,

    -- unicidade da “chave” da nota
    CONSTRAINT UQ_NOTA_SAIDA_CHAVE
        UNIQUE (NOTA_SAIDA_MODELO, NOTA_SAIDA_SERIE, NOTA_SAIDA_NUMERO, CLIENTE_ID),
);
GO
...
CREATE TABLE ITENS_NOTA_SAIDA (
    ...
    -- Calculado: total da linha (espelha o get { qtd * unit })
    ITEM_NOTA_VLR_TOTAL   AS (ITEM_NOTA_QTD * ITEM_NOTA_VLR_UNIT) PERSISTED,

    -- Auditoria (mesmo padrão de ITENS_NOTA_ENTRADA)
    ATIVO                 BIT           NOT NULL DEFAULT 1,
    ITEM_NOTA_DT_CRIACAO  DATETIME      NOT NULL DEFAULT GETDATE(),
    ITEM_NOTA_DT_ALT      DATETIME      NULL,
    ITEM_NOTA_USR_ALT     INT           NULL
);
GO
...
ALTER TABLE NOTA_SAIDA
  ADD CONSTRAINT FK_NOTASAIDA_CLIENTE
          FOREIGN KEY (CLIENTE_ID)
          REFERENCES CLIENTE (CLIENTE_ID),
      CONSTRAINT FK_NOTASAIDA_CONDPAG
          FOREIGN KEY (CONDPAG_ID)
          REFERENCES COND_PAGAMENTO (CONDPAG_ID),
      CONSTRAINT FK_NOTASAIDA_USR_ALT
          FOREIGN KEY (NOTA_SAIDA_USR_ALT)
          REFERENCES FUNCIONARIO (FUNCIONARIO_ID),
    CONSTRAINT FK_NOTA_SAIDA_FUNCIONARIO
...
ALTER TABLE CONTAS_A_RECEBER
  ADD CONSTRAINT FK_ContasAReceber_Cliente
      FOREIGN KEY (ID_CLIENTE)
          REFERENCES CLIENTE (CLIENTE_ID);
GO

-- FK para FORMA_PAGAMENTO
ALTER TABLE CONTAS_A_RECEBER
  ADD CONSTRAINT FK_ContasAReceber_FormaPagamento
      FOREIGN KEY (ID_FORMA_PAGAMENTO)
          REFERENCES FORMA_PAGAMENTO (FORMAPAG_ID);
GO

ALTER TABLE CONTAS_A_RECEBER
  ADD CONSTRAINT FK_ContasAReceber_NotaSaida
      FOREIGN KEY (NOTA_SAIDA_ID)
      REFERENCES NOTA_SAIDA (NOTA_SAIDA_ID)
GO

ALTER TABLE ITENS_NOTA_SAIDA
  ADD CONSTRAINT FK_ITEMNOTASAIDA_NOTASAIDA
      FOREIGN KEY (NOTA_SAIDA_ID)
      REFERENCES NOTA_SAIDA (NOTA_SAIDA_ID)
GO

ALTER TABLE ITENS_NOTA_SAIDA
      ADD CONSTRAINT FK_ITEMNOTASAIDA_PRODUTO
          FOREIGN KEY (PRODUTO_ID)
          REFERENCES PRODUTO (PRODUTO_ID);
GO

ALTER TABLE ITENS_NOTA_SAIDA
    ADD CONSTRAINT FK_ITEMNOTASAIDA_USR_ALT
        FOREIGN KEY (ITEM_NOTA_USR_ALT)
        REFERENCES FUNCIONARIO (FUNCIONARIO_ID);
GO
...
CREATE UNIQUE INDEX UX_ContasAReceber_NotaSaida_Parcela
    ON CONTAS_A_RECEBER (NOTA_SAIDA_ID, NUMERO_PARCELA)
    WHERE NOTA_SAIDA_ID IS NOT NULL;
GO

-- Índices úteis para consultas por vencimento e status
CREATE INDEX IX_ContasAReceber_DataVencimento
    ON CONTAS_A_RECEBER (DATA_VENCIMENTO);
GO

CREATE INDEX IX_ContasAReceber_Situacao
    ON CONTAS_A_RECEBER (SITUACAO);
GO
